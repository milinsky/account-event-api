...
build: ## Create containers
	...
	docker compose run -w /var/www/api cli composer install
	docker compose run -w /var/www/worker cli composer install
	docker compose run -w /var/www/tank cli composer install
